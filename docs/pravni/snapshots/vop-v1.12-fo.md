# Všeobecné obchodní podmínky (VOP)

**Platforma:** zaPikolou.cz  
**Verze:** 1.12-fo · **Datum účinnosti:** 03.09.2026  
**Provozovatel:** Radek Horák · kontaktní e-mail: info@zapikolou.cz · datová schránka: 8q4nyyt

Údaje o provozovateli (jméno, sídlo, IČO, zápis v rejstříku, e-mail, datová schránka) najdete na stránce [Provozovatel webu](/kontakt).

---

## TL;DR — v čem je pointa

Jsme **inzertní nástěnka, ne prodejce ani zprostředkovatel obchodu**. Za obsah inzerátů a průběh dohod mezi uživateli odpovídají sami inzerenti a zájemci. My pouze technicky propojujeme nabídku a poptávku.

---

## 1. Status platformy a limitace odpovědnosti

### 1.1 Zprostředkovatelský charakter služeb

Provozovatel provozuje platformu zaPikolou.cz jako hostingovou službu, která umožňuje registrovaným uživatelům zveřejňovat inzeráty — včetně nabídky a poptávky **zboží, služeb, pracovních příležitostí, nemovitostí a událostí**. Návštěvník bez účtu může připravit koncept inzerátu (včetně nahrání fotografií a náhledu automatické kontroly); **zveřejnění** inzerátu je možné až po registraci a přihlášení. Provozovatel **není** prodejcem, kupujícím, zaměstnavatelem, pronajímatelem ani jiným účastníkem transakcí mezi uživateli.

### 1.2 Vyloučení odpovědnosti

Provozovatel nenese odpovědnost za kvalitu, stav, bezpečnost, legálnost ani pravdivost **nabízeného obsahu** (včetně zboží, služeb, pracovních příležitostí, nemovitostí a událostí), ani za **plnění dohodnutých podmínek** mezi uživateli (včetně zaplacení ceny, odměny, nájmu nebo jiného plnění). Odpovědnost nese plně inzerent a zájemce.

### 1.3 Limitace náhrady škody

Provozovatel neodpovídá za ušlý zisk či následné škody způsobené výpadky systému nebo kybernetickými útoky. Inzertní služba je poskytována **bezúplatně**; odpovědnost Provozovatele za škodu je omezena v rozsahu, v jakém to kogentní ustanovení právních předpisů dovolují.

### 1.4 Nemovitosti

Provozovatel **nevykonává činnost realitního zprostředkovatele** ve smyslu zákona o realitním zprostředkování. Platforma umožňuje pouze zveřejnění inzerátů týkajících se nemovitostí; smlouvy uzavírají výhradně uživatelé mezi sebou. Provozovatel se neúčastní vyjednávání, nevybírá provizi z transakce a neověřuje vlastnictví nemovitostí.

---

## 2. Technické parametry a reklamace

### 2.1 Reklamace

Inzertní služba je poskytována **bezúplatně**. Výpadek, odstávka nebo omezení provozu **nezakládá** právo na peněžitou náhradu, obnovu kreditů ani jiné peněžité plnění.

Uživatel může technický problém oznámit na kontaktní e-mail Provozovatele. Provozovatel se jej pokusí odstranit v přiměřené době; **nevzniká** nárok na slevu, náhradu škody nad rámec čl. 1.3 ani na navýšení limitu publikací.

### 2.2 Výluky

Za vadu se nepovažují:

- plánované odstávky oznámené předem,
- zásahy vyšší moci,
- výpadky u dodavatelů internetového připojení nebo třetích stran mimo kontrolu Provozovatele.

### 2.3 Inzertní služby a limity

**Inzertní službou** se rozumí plnění Provozovatele spočívající v umožnění zveřejnění inzerátu na platformě. Služba je poskytována **bezúplatně** v rozsahu limitu publikací; pravidla limitů a spotřeby kreditů (lifetime model) jsou na stránce [Limity inzerce](/balicky-inzerce).

Konkrétní **limity publikací a pravidla spotřeby kreditů** jsou vedeny mimo tento dokument na uvedeném odkazu. Provozovatel je oprávněn parametry limitů aktualizovat se zveřejněním na webu. Uživatelům inzerujícím jako Podnikatelé musí být každá změna limitů, která pro ně znamená zhoršení podmínek, oznámena e-mailem nejméně **15 dní** před plánovanou účinností této změny. Pokud Podnikatel v této lhůtě s novými podmínkami nesouhlasí, má právo smlouvu vypovědět a inzerci bez sankcí ukončit. U ostatních uživatelů platí zveřejnění na uvedeném odkazu; již přidělené kredity zůstávají v platnosti.

Kredit se vztahuje na **jednu konkrétní nabídku**. Běžné úpravy téže nabídky kredit znovu nespotřebovávají. Platnost včetně prodloužení je nejvýše **365 dní od založení** inzerátu (technický strop životnosti na platformě, **nikoli** garance nepřetržitého provozu — viz čl. 8.3) — poté inzerát zaniká (viz [Limity inzerce](/balicky-inzerce)). Záměrná výměna obsahu inzerátu za **jinou** nabídku (zneužití lifetime modelu) je zakázána [Pravidly inzerce](/podminky-inzerce); Provozovatel může v takovém případě postupovat podle čl. 4 těchto VOP (zejména zablokování nebo odstranění inzerátu) a požadovat novou publikaci.

---

## 3. Práva k obsahu a ochrana značky

### 3.1 Autorská práva

Uživatel prohlašuje, že je autorem nebo oprávněným uživatelem všech vložených fotografií a textů. Uživatel nese plnou odpovědnost za případné zásahy do práv třetích osob.

### 3.2 Licence k obsahu

Vložením fotografie nebo jiného autorského díla k inzerátu uděluje Uživatel Provozovateli bezúplatnou, nevýhradní licenci k jeho užití — zejména k zobrazení, kompresi, změně formátu a případnému vkládání vodoznaku dle čl. 3.3 — a to po dobu zveřejnění inzerátu na platformě. Licence zaniká spolu s odstraněním inzerátu, s výjimkou technických kopií nezbytných pro zálohování nebo plnění právních povinností.

### 3.3 Úpravy a vodoznaky

Provozovatel si **vyhrazuje právo** (nikoli povinnost) vkládat do fotografií vodoznak s logem zaPikolou a automaticky upravovat formátování textu (např. převod textu psaného velkými písmeny na malá), pokud to slouží jednotnému zobrazení nebo ochraně platformy. Vodoznak Provozovatel **nepoužívá povinně** — rozhodnutí o jeho použití závisí na technických a provozních potřebách platformy.

### 3.4 Zákaz recyklace cizího obsahu

Uživatel nesmí k vlastnímu inzerátu připojit fotografie, které pořídil nebo použil jiný uživatel k odlišné nabídce téže nebo obdobné věci. Porušení může vést k odstranění inzerátu bez nároku na náhradu.

---

## 4. Moderace obsahu (v souladu s DSA)

### 4.1 Automatizovaná kontrola obsahu (AI)

Každý inzerát před zveřejněním prochází **automatizovanou kontrolou obsahu**, a to včetně všech nahraných fotografií. Kontrola může zahrnovat předfiltraci fotografií (např. detekce nahoty) a následné posouzení textu a fotografií systémem umělé inteligence. Inzerát je zveřejněn teprve po úspěšném absolvování této kontroly. Automatizovaná kontrola posuzuje soulad se zákonem, s těmito VOP a s [Pravidly inzerce](/podminky-inzerce), zejména s aktuálním seznamem zakázaného obsahu dle čl. 4.2.

### 4.2 Seznam zakázaného obsahu

Konkrétní kategorie zakázaného nebo omezeného obsahu (např. jednotlivé druhy zboží, služeb či klíčových slov) jsou vedeny v [Pravidlech inzerce](/podminky-inzerce) mimo tento dokument. Provozovatel je oprávněn tento seznam **kdykoli aktualizovat** s okamžitou účinností — typicky v reakci na nové způsoby zneužití platformy — bez nutnosti současné změny těchto VOP. Aktuální znění je vždy dostupné na uvedeném odkazu.

**Absolutní zákazy** včetně zvlášť závažného obsahu jsou v [Pravidlech inzerce](/podminky-inzerce) čl. 2.4; sankce a postup v čl. 4.5 těchto VOP.

### 4.3 Lidský přezkum (God Mode)

Automatizovaná kontrola je doplněna lidským dohledem. Moderátor nebo administrátor Provozovatele (dále jen „Moderátor"):

- rozhoduje o obsahu, který se dostane do karantény na základě hlášení (čl. 4.4) nebo automatizované kontroly, a má právo takový obsah obnovit, ponechat zablokovaný nebo trvale odstranit,
- může kdykoli **z vlastního podnětu** inzerát skrýt nebo odstranit i bez předchozího hlášení či bez zásahu automatizované kontroly, pokud vyhodnotí rozpor se zákonem, těmito VOP nebo Pravidly inzerce,
- má vždy poslední slovo — automatizovaná kontrola ani automatické zablokování na základě hlášení nejsou konečným rozhodnutím tam, kde věc putuje k lidskému přezkumu.

### 4.4 Hlášení obsahu

Kdokoli může nahlásit nelegální inzerát prostřednictvím formuláře [Nahlásit inzerát](/nahlasit). Postup a kontaktní údaje jsou popsány v [DSA kontaktním centru](/dsa).

### 4.5 Odstraňování a omezení viditelnosti

Provozovatel může inzerát **odstranit** nebo **zablokovat** (skrýt z veřejného zobrazení), pokud porušuje zákon nebo tato pravidla a [Pravidla inzerce](/podminky-inzerce).

| Opatření | Význam pro inzerenta |
|----------|----------------------|
| **Zablokování** | Inzerát není veřejně viditelný; obnovení vyžaduje úpravu obsahu a nové schválení |
| **Odstranění** | Inzerát je trvale smazán (soft delete) |

Provozovatel si vyhrazuje právo **zrušit automatické zablokování** i bez úpravy obsahu inzerátu, pokud vyhodnotí, že hlášení byla zjevně nedůvodná, účelová nebo koordinovaná (např. zneužití hlášení proti konkurenci).

**Hard stop — CSAM a závažné zneužití:** Při pokusu nahrát nebo šířit obsah související se sexuálním zneužíváním dětí (CSAM), nebo při opakovaných pokusech obejít moderaci / nahrát zakázaný sexuální či jinak zvlášť závažný obsah, je Provozovatel oprávněn **okamžitě pozastavit nebo zrušit uživatelský účet** (včetně všech inzerátů) **bez předchozí výzvy**, a to i u Podnikatele. Lhůta 30 dní dle odstavce níže se v těchto případech neuplatní. Provozovatel může obsah uchovat v omezené evidenci pro plnění zákonných povinností a v zákonem stanovených případech věc oznámit příslušným orgánům.

**Úplné ukončení služeb Podnikateli:** Pokud se Provozovatel rozhodne zcela zrušit nebo trvale pozastavit uživatelský účet Podnikatele (nikoli pouze skrýt jednotlivý inzerát), je povinen poskytnout Podnikateli e-mailem odůvodnění tohoto rozhodnutí nejméně **30 dní** před tím, než ukončení nabude účinnosti. Lhůta 30 dní neplatí, pokud Podnikatel prokazatelně opakovaně porušuje tyto VOP nebo Pravidla inzerce, jedná protiprávně, pokud jde o CSAM či obdobně závažné porušení dle předchozího odstavce, nebo pokud okamžité zrušení účtu vyžaduje zákon či rozhodnutí dozorového úřadu.

### 4.6 Odůvodnění opatření

O každém odstranění nebo omezení viditelnosti inzerátu zašle Provozovatel dotčenému uživateli e-mailem **jasné odůvodnění** (Statement of Reasons dle DSA), včetně právního nebo smluvního základu a možnosti podání stížnosti dle čl. 4.7.

### 4.7 Stížnosti a odvolání

Uživatel, jehož inzerát nebo účet byl dotčen opatřením dle čl. 4.5, se může **odvolat nebo podat stížnost** prostřednictvím [DSA kontaktního centra](/dsa) nebo na kontaktní e-mail Provozovatele uvedený v čl. 8. Provozovatel stížnost posoudí v přiměřené lhůtě a o výsledku informuje uživatele. U Podnikatelů mohou platit doplňující požadavky dle čl. 7.3.

---

## 5. Mimosoudní řešení sporů (ADR)

V případě sporu, který se nepodaří vyřešit dohodou, se spotřebitel může obrátit na **Českou obchodní inspekci (ČOI)** — [adr.coi.cz](https://adr.coi.cz).

> **Poznámka:** Evropská platforma ODR byla k 20. 3. 2025 zrušena a není již k dispozici.

---

## 6. Věk a způsobilost k užívání

Platforma je určena uživatelům od **15 let** věku.

Registrací prohlašujete, že vám je **alespoň 15 let**. Jste-li ve věku **15–18 let**, jste si vědomi, že některé právní úkony související s inzerací (zejména v kategoriích **nemovitostí** — pronájem nebo prodej) mohou vyžadovat souhlas zákonného zástupce dle občanského zákoníku.

Registrace osob **mladších 15 let** není dovolena. U osob mladších 15 let může být souhlas se zpracováním osobních údajů v souvislosti s informační společností udělen pouze zákonným zástupcem — viz [Zásady ochrany osobních údajů](/gdpr).

---

## 7. Inzerce podnikatelů a právnických osob

### 7.1 Přípustnost podnikatelské inzerce

Platforma umožňuje inzerci fyzickým osobám i podnikatelům/právnickým osobám. Uživatel je povinen při **nastavení profilu** (registraci) pravdivě uvést, zda inzeruje jako **soukromá osoba**, nebo **v rámci své podnikatelské či jiné výdělečné činnosti** (dále jen „Podnikatel").

### 7.2 Označení Podnikatele

Označí-li se Uživatel při nastavení profilu jako Podnikatel, je povinen uvést **obchodní firmu/název** a **IČO** (pokud mu bylo přiděleno). U jeho inzerátů se pak viditelně zobrazí štítek „Podnikatel". Toto označení slouží k tomu, aby zájemce věděl, že jde o nabídku od podnikatele, a nikoliv od soukromé osoby — od čehož se mohou odvíjet odlišná práva zájemce jako spotřebitele, včetně případného práva na odstoupení od smlouvy, pokud jsou splněny zákonné podmínky. Tato práva se neuplatní při obchodu mezi dvěma soukromými osobami.

### 7.3 Vztah k nařízení (EU) 2019/1150 (P2B)

Pokud Podnikatel používá platformu k nabízení zboží nebo služeb spotřebitelům, vztahuje se na vztah mezi Provozovatelem a Podnikatelem nařízení (EU) 2019/1150 — zejména právo na odůvodnění při omezení nebo odstranění inzerátu (viz čl. 4.6), lhůty při změně podmínek (viz čl. 2.3 a 8.2), předchozí oznámení při zrušení účtu (viz čl. 4.5) a parametry řazení inzerátů (viz čl. 7.4).

Provozovatel prohlašuje, že splňuje kritéria **mikropodniku** ve smyslu doporučení Komise 2003/361/ES. V souladu s článkem 11 odst. 5 a článkem 12 odst. 7 nařízení (EU) 2019/1150 se na vztah mezi Provozovatelem a Podnikatelem **neuplatňuje** povinnost zřízení interního systému pro vyřizování stížností, ani povinnost určovat nezávislé mediátory pro řešení případných mimosoudních sporů. Pro řešení stížností Podnikatelů se použije standardní postup prostřednictvím kontaktů uvedených v čl. 8 a obecné mechanismy podle čl. 4.7 těchto VOP.

### 7.4 Parametry řazení inzerátů

Hlavními parametry, které určují pořadí zobrazování inzerátů na platformě (včetně výsledků vyhledávání), jsou:

1. **Geografická blízkost:** Systém upřednostňuje inzeráty lokalizované co nejblíže k poloze zadané nebo detekované u návštěvníka (v adaptivních krocích 15, 30, 50 až 60 km).
2. **Aktualita (čas):** V rámci stejného geografického okruhu nebo při celostátním zobrazení jsou inzeráty řazeny chronologicky od nejnovějšího (podle data publikace nebo obnovení).

Platforma neumožňuje Podnikatelům přímé placené ovlivňování (tzv. topování nebo umělé zvyšování pozice) inzerátu výměnou za odměnu, které by mělo vliv na toto základní řazení.

### 7.5 Přístup k vygenerovaným datům

Každý uživatel s aktivním účtem má u svých vlastních inzerátů přístup k **agregovaným statistikám** (zejména k celkovému počtu **zobrazení inzerátu** v sekci [Moje inzeráty](/moje-inzeraty)) — stejně soukromá osoba i Podnikatel. Poptávky od zájemců se doručují e-mailem. Uživatel nemá přístup k identifikačním údajům jednotlivých prohlížečů ani k osobním údajům jiných uživatelů platformy, s výjimkou situace, kdy zájemce sám odešle své kontaktní údaje prostřednictvím poptávkového formuláře. Po smazání nebo anonymizaci účtu jsou všechna tato data trvale nedostupná. Provozovatel tato metadata v anonymizované podobě využívá pro účely provozní analytiky.

---

## 8. Závěrečná ustanovení

### 8.1 Související dokumenty

K používání platformy patří rovněž tyto dokumenty (v aktuálním znění na webu):

| Dokument | URL |
|----------|-----|
| [Pravidla inzerce](/podminky-inzerce) | Kodex chování, zakázaný obsah, moderace |
| [Limity inzerce](/balicky-inzerce) | Limity publikací, inzertní služby |
| [Zásady ochrany osobních údajů](/gdpr) | GDPR |
| [DSA kontaktní centrum](/dsa) | Hlášení obsahu, stížnosti |

### 8.2 Aktualizace a souhlas

Registrací nebo používáním platformy uživatel potvrzuje, že se seznámil s těmito VOP a souhlasí s nimi. Provozovatel může VOP aktualizovat. O **podstatných změnách** těchto VOP informuje Provozovatel uživatele e-mailem. Pro uživatele s profilem Podnikatele platí pevná lhůta **15 dní** od oznámení do nabytí účinnosti změn VOP. Lhůta se neuplatní pouze v případě, kdy Provozovatel mění podmínky na základě okamžité zákonné povinnosti nebo k odvrácení nepředvídaného kybernetického či podvodného rizika. Drobné technické úpravy, opravy překlepů a odkazy na limity inzerce či seznamy zakázaného obsahu (čl. 2.3, 4.2) podstatnou změnou nejsou, pokud nemění práva a povinnosti uživatele v zásadním rozsahu.

### 8.3 Ukončení nebo omezení provozu platformy

Provozovatel je oprávněn provoz platformy **kdykoli ukončit, pozastavit nebo podstatně omezit** (včetně trvalého vypnutí). Maximální platnost inzerátu dle čl. 2.3 (**nejvýše 365 dní**) je pouze technický strop — **nezakládá** právo na dostupnost inzerátu ani na provoz platformy po celou tuto dobu.

Ukončením nebo podstatným omezením provozu zaniká možnost zveřejnění a prohlížení inzerátů. U bezplatné inzerce nevzniká uživateli nárok na náhradu, obnovu kreditů ani jiné peněžité plnění.

O plánovaném ukončení Provozovatel uživatele informuje přiměřeně předem (typicky e-mailem), pokud to okolnosti dovolí. U uživatelů s profilem Podnikatele se v rozsahu vyžadovaném nařízením (EU) 2019/1150 uplatní příslušné oznamovací lhůty.

**Kontakt:** info@zapikolou.cz · datová schránka: 8q4nyyt · [Provozovatel webu](/kontakt)

---
