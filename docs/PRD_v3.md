...
3. **Ochrana kontaktů:** V HTML zdroji detailu inzerátu **není** telefon ani e-mail před kliknutím na „Zobrazit kontakt“ (ověřitelné v DevTools). `posts.contact_phone` (a pro `anon` i `location` / `original_*`) není v column-level SELECT grantu (migrace `078`; samotný `REVOKE` sloupce po table `GRANT SELECT` z `025` Postgres ignoruje). E-mail cizího profilu není enumerovatelný přes RLS. Odhalení jde výhradně přes RPC `reveal_listing_contact` (přihlášení, viditelnost inzerátu, opt-in, rate limit 20/den).
...
* **Analytika:** Google Tag Manager (GTM) + Google Analytics 4 (GA4) s **vlastní cookie lištou** (GTM Consent Mode v2) před aktivací měření. *(✅ GTM `GTM-WGLNJRNK`, consent banner, `/cookies` — 2026-07-14; konverze publikace = Custom Event `generate_lead` — 2026-08-27)*
...
posts
  ...
  - campaign_attribution (JSONB, nullable — first-touch UTM / `fbclid` / `gclid` při založení; migrace `086`; **není** v REST SELECT; God Mode přes service_role)
...
* **Guest draft (Funnel C, v3.64 / prod v3.94):** Nepřihlášený vyplní `/inzerat/novy` (fotky → AI) bez loginu; účet až při Publikovat. Flag `NEXT_PUBLIC_GUEST_LISTING_DRAFT_ENABLED` — v kódu default vypnuto, **na produkci zapnuto** (ověřeno 2026-09-01). Site-wide, ne jen z FB. Detail: [`fb-promo-campaign.md`](./fb-promo-campaign.md).

* **AI photo-first prefill (zboží, v3.61 / UX v3.74 / fallback v3.81 / jazyk v3.99 / nápis na věci v3.102):** Na `/inzerat/novy` je krok 0 — karta „Pro začátek stačí dvě fotky.“; CTA **Předvyplnit inzerát** (ne „Vytvořit inzerát“). Mobil **Vyfotit** + **Vybrat z galerie** (1–2 fotky), desktop dropzone; viditelná manuální cesta pro služby/události/práci/reality. Edge `suggest-listing-from-photos` nejprve provede Sightengine NSFW gate a teprve po jeho úspěchu volá Gemini; při timeoutu, HTTP chybě, blokaci nebo chybě parse zkusí OpenAI. Closed vocabulary z `GOODS_CATEGORIES`; výstup předvyplní `title` / `description` / `categoryType` / `subcategorySlug`. **Bez** odhadu ceny, stavu a lokality — po Prefillu jsou prázdné a žlutě zvýrazněné, dokud je uživatel nevyplní (stav = „— vyberte —“). Nejisté atributy v popisu jako `Doplňte značku: `. Čitelný nápis modelu na věci (štítek, panel, hlava nástroje, víko motoru) se zapíše do názvu, ne jako `Doplňte typ/model:`. `Doplňte …:` jen na údaj, který v názvu/nabídce ještě není. Výbavu jen z fotky, ne z produktové řady. Země výroby a sériové číslo do draftu ne. Čitelný štítek velikosti (kg) se zapíše do nabídky, ne jako prázdné `Doplňte velikost:`. Jazyk: běžná inzerátní čeština (overal/kombinéza, ne kombinezon). Primary model řídí `SUGGEST_LISTING_MODEL` (default `gemini-3.5-flash-lite`, max. 12 s), fallback `SUGGEST_FALLBACK_MODEL` (default `gpt-5.4-nano`, max. 8 s); `MODERATION_FINAL_*` a `OPENAI_MODERATION_MODEL` patří pouze publish moderaci. Když chybí Gemini klíč, OpenAI je primary (`used_fallback = false`); dvojí selhání vrací technickou 503 a ruční cesta zůstává dostupná. Auth JWT nebo guest visitor; rate limit se počítá jednou na request. Flag `SUGGEST_FROM_PHOTOS_ENABLED`. Publish gate zůstává `moderate-listing` beze změny. **Staff prefill lab (v3.65 / hydratace v3.99):** `/mod/prefill-lab` + Edge `compare-suggest-from-photos` — stejné suggest prompt/schema, dvě sekvenční volání; bez DB / Sightengine u A/B. Druhý krok **Hydratovat** volá produkční `moderate-listing` (preview) — zápis do `moderation_checks` a rate limit jako u Publikovat.

* **Multimodální AI Guardrail & Interaktivní doplňování (Text + Foto cross-validace):**
  * Po kliknutí na **„Publikovat inzerát“** (create) / **„Uložit“** (edit): klient nejprve nahraje nové originály do privátního immutable stagingu (`moderation-image-staging`); Next.js Server Action přes **Sharp** vytvoří hash-addressed WebP varianty do `moderation-image-renditions` (**Gemini až 1920 px**, **Sightengine 512 px**, `fit: inside` — zachovaný poměr stran, WebP kvalita 80 s fallbackem pod 1 MB). Pak klient zavolá **přímo** Edge Function `moderate-listing` přes `supabase.functions.invoke()` (JWT uživatele v hlavičce). Payload: `title`, surový popis, `categoryType`, `subcategory_slug`, metadata z formuláře (`conditionLabel`, `conditionLabelText`, `conditionFieldLabel`, `priceType`, `priceTypeLabel`, `priceAmount`, u událostí `eventDate`), **`imageReferences`** (max. 6 odkazů na staging objekty — **ne** base64) a `mainImageIndex`. Edge stáhne originály (SEC-H02 hashe), načte důvěryhodné varianty a provede **jedno** AI kolo — Sightengine NSFW na všech snímcích (512 px), cross-validace text ↔ hlavní fotka + hydratace z **všech** fotek přes Gemini (1920 px) / GPT-4o-mini fallback; výstup je striktní JSON. **Žádná Next.js API Route v AI volání** (Sharp Server Action jen připravuje varianty). Detail: [`moderace-inzeratu.md`](./moderace-inzeratu.md).
...
* **AI modely — kanonická tabulka defaults (aktualizace 2026-09-07):**
...
Prefill nepoužívá `MODERATION_FINAL_*` ani `OPENAI_MODERATION_MODEL`. Gemini má rozpočet 12 s a OpenAI fallback 8 s; Sightengine musí uspět před oběma. Prefill lab A/B je staff-only srovnání (bez DB); tlačítko **Hydratovat** jde přes `moderate-listing` preview. UI defaulty v `src/config/compare-suggest-from-photos.ts`. Při změně modelu aktualizujte **datum v této tabulce**, Metodiku §6 a changelog níže.
...
| v3.61 | 2026-08-08 | **AI photo-first prefill (zboží):** krok 0 na `/inzerat/novy`; Edge `suggest-listing-from-photos` (NSFW + Gemini); předvyplní title/description/category; migrace `074` rate limit `suggest_from_photos`; `moderate-listing` beze změny |
...
| v3.102 | 2026-09-07 | **Prefill nápis na věci:** čitelný model (štítek, panel, hlava nástroje, víko motoru) do názvu, ne `Doplňte typ/model:`. `Doplňte` jen na chybějící údaj. Výbava jen z fotky. Bez Made in / sériového čísla. Edge `suggest-listing-from-photos` + lab. |
| v3.103 | 2026-09-08 | **UTM u inzerátu:** first-touch `posts.campaign_attribution` při založení (ne při editaci). Migrace [`086`](../supabase/086_posts_campaign_attribution.sql) — mimo REST SELECT. God Mode: sloupec Zdroj + detaily v liště na detailu. |
