...
* **AI modely — kanonická tabulka defaults (aktualizace 2026-08-25):**
...
Prefill nepoužívá `MODERATION_FINAL_*` ani `OPENAI_MODERATION_MODEL`. Gemini má rozpočet 12 s a OpenAI fallback 8 s; Sightengine musí uspět před oběma. Prefill lab je staff-only srovnání (bez DB); UI defaulty v `src/config/compare-suggest-from-photos.ts`. Při změně modelu aktualizujte **datum v této tabulce**, Metodiku §6 a changelog níže.
