...
Klient nové soubory jednou nahraje do privátního immutable bucketu `moderation-image-staging`. Autentizovaná Next.js Server Action přes Sharp vytvoří WebP varianty všech fotek (Gemini až 1920 px, Sightengine 512 px, kvalita 80 s fallbackem pod 1 MB) a uloží je pod SHA-256 originálu do privátního `moderation-image-renditions` (`gemini-1920.webp` / `sightengine-512.webp`). Do Edge klient posílá jen `imageReferences` + `mainImageIndex`; Edge sama stáhne plné objekty, spočítá SHA-256 pro SEC-H02 a podle něj načte důvěryhodné varianty. Při publikaci Server Action zkopíruje stejné bajty do `post-images` a publish gate je znovu zahashuje. Hvězdička u miniatury = **náhled na homepage**, ne „jediná kontrolovaná fotka“.

Po změně názvu rendice (např. `gemini.webp` → `gemini-1920.webp`) nasazuj **nejdřív Vercel**, až potom Edge (`moderate-listing`, `suggest-listing-from-photos`, `compare-suggest-from-photos`). Obráceně Edge hledá nové jméno, které Next ještě nezapisuje → 503 `IMAGE_RENDITION_MISSING`.
